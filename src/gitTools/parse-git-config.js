...
/**
 * Asynchronously parse a `.git/config` file. If only the callback is passed, the `.git/config` file
 * relative to `process.cwd()` is used.
 *
 * ```js
 * parse((err, config) => {
 *   if (err) throw err;
 ...
 * ```
 *
 * @param {Object | String | Function} `options` Options with `cwd` or `path`, the cwd to use, or
...
/**
 * Synchronously parse a `.git/config` file. If no arguments are passed, the `.git/config` file
 * relative to `process.cwd()` is used.
 *
 * ```js
